...
from `runner_scripts/problems.csv` (the sweep grid is rounded through
float32):

- `golden_<problem>_131072.csv` — 131072 rows of final states; the
  work-precision (wp) reference. Generated by
  `runner_scripts/golden/generate_golden.jl`; an existing file is kept
  unless `--force`.
- `golden_ne_<problem>_1024.csv` — 1024 rows `<sweep>,<states...>`; the
  numerical-equivalence (ne) reference. The first column carries the
  float32-rounded parameter grid so every ne consumer integrates
  bit-identical Float32 inputs. Generated by
