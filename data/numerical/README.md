...
Two machine-independent Float64 golden files also live here (CPU Vern9 at
abstol = reltol = 1e-13, rho linspace rounded through float32):

- `golden_lorenz_131072.csv` — 131072 rows `x,y,z`; the work-precision (wp)
  reference. Generated by `runner_scripts/golden/generate_golden.jl`.
- `golden_ne_lorenz_1024.csv` — 1024 rows `rho,x,y,z`; the
  numerical-equivalence (ne) reference. The rho column carries the
