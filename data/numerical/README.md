...
Machine-independent Float64 golden files also live here, one pair per
problem, solved with the problem's `golden_algorithm` at its `golden_tol`
from `runner_scripts/problems.csv`:

- `golden_<problem>_131072.csv` — 131072 rows of final states; the
  work-precision (wp) reference. Generated by
  `runner_scripts/golden/generate_golden.jl`; existing files are kept
  unless `--force`.
- The numerical-equivalence (ne) ensemble is the first 1024 points of the wp
  sweep, scored on the first 1024 rows of `golden_<problem>_131072.csv`.
- `golden_ne_<problem>_1024.csv` — 1024 rows `<sweep>,<states...>`; the ne
  reference and grid of the problems listed under `ne_legacy_grid` in
  `runner_scripts/protocol.toml`. Generated by
