...
with open(os.path.join(data_dir("CUBIE_MLIR", DATASET_KEY), "Cubie_mlir_times_unadaptive.txt"), "a+") as file:
...
if numberOfParameters == 32768:
    ...
    np.savetxt(os.path.join(data_dir("numerical", DATASET_KEY), "cubie_mlir_unadaptive.csv"), final_states, delimiter=',')
...
# Save results
with open(os.path.join(data_dir("CUBIE_MLIR", DATASET_KEY), "Cubie_mlir_times_adaptive.txt"), "a+") as file:
    file.write(f'{numberOfParameters} {best_time} {best_time_dev}\n')
...
if numberOfParameters == 32768:
    ...
    # Extract final state values
    final_states = solution.state[-1, :, :].T  # shape: (trajectories, states)
    np.savetxt(os.path.join(data_dir("numerical", DATASET_KEY), "cubie_mlir_adaptive.csv"), final_states, delimiter=',')
