...
All timed samples are synchronized end-to-end solves, including final host transfer. Each point has an untimed warmup. Fixed performance uses dyadic `dt=2^-10`; full numerical equivalence uses N=1024 with `dt=2^-1..2^-13` and tolerances `1e-2..1e-8` (the cubie side is imported from the NE suite's outputs rather than re-solved, and explicit fixed sweeps are excluded as bit-equivalent); full work-precision uses N=131072 with `dt=2^-4..2^-13`, tolerances `1e-2..1e-8`, and duration-scheduled repeats (20 for legs whose first timed run is under 100 ms; slower legs run a floor of 10/5/3 and extend toward 10 while median/min − 1 stays above 2%). Each transfer variant runs as an unbroken block of repeats, and the workers reduce those repeats to one row per point before writing. Reported runtimes are the minimum over repeats, matching the main performance suite; `min_ms`, `p05_ms`, `median_ms`, `p95_ms` and `max_ms` describe the spread behind it.
