"""cubie_precompile.py: the kernel listing, the chunking and the driver loop against a fake worker script: chunks of per-worker kernels to at most --jobs workers at once, a worker's hard exit or crash requeuing the rest of its chunk, failed kernels tallied, a worker dead before its first kernel failing the pass."""
...
class ListingTests(unittest.TestCase):
    def test_kernel_lines_keep_the_first_line_of_every_kernel_in_file_order(self):
        built = trials.build_trials([spec(8), spec(32), spec(8, dt=2.0 ** -12), spec(8, algorithm="euler"),
                                     spec(8, controller="default", dt=2.0 ** -10, atol=1e-5, rtol=1e-5),
                                     spec(8, controller="default", dt=2.0 ** -10, atol=1e-6, rtol=1e-6)])
        lines = trials.kernel_lines(built)
        self.assertEqual([(t["algorithm"], t["controller"], t["atol"] if t["atol"] == t["atol"] else None, t["n"])
                          for t in lines],
                         [("euler", "fixed", None, 8), ("tsit5", "default", 1e-5, 8), ("tsit5", "default", 1e-6, 8),
                          ("tsit5", "fixed", None, 8)])
        self.assertEqual(trials.kernels_of(built), 4)
        self.assertEqual(len({trials.kernel_key(t) for t in lines}), 4)

    def test_chunks_labels_and_flags(self):
        self.assertEqual(cubie_precompile.chunks(10, 3), [(0, 3), (3, 6), (6, 9), (9, 10)])
        self.assertEqual(cubie_precompile.chunks(0, 3), [])
        self.assertEqual(cubie_precompile.chunks(8, 8), [(0, 8)])
        self.assertEqual(cubie_precompile.kernel_label(spec()), "lorenz tsit5 fixed dt=0.000976562")
        self.assertEqual(cubie_precompile.kernel_label(spec(controller="default", atol=1e-5)), "lorenz tsit5 default tol=1e-05")
        self.assertEqual(cubie_precompile.kernel_label(spec(dt=NAN, system_params='{"states": 8}', problem="lorenz96")),
                         'lorenz96 tsit5 fixed {"states": 8}')
        ...
        for bad in (["--trials", "x", "--worker", "3"], ["--trials", "x", "--jobs", "0"], ["--precompile"]):
            with self.assertRaises(SystemExit):
                cubie_precompile.parse_args(bad)
        self.assertEqual(cubie_precompile.progress_path("t.jsonl", 3), "t.jsonl.precompile3.progress")
        argv = cubie_precompile.worker_argv_for("bench_cubie.py", "t.jsonl")(3, 6)
        self.assertEqual(argv, [sys.executable, "bench_cubie.py", "--trials", "t.jsonl", "--precompile", "--worker", "3:6"])
...
class DriverTests(unittest.TestCase):
    def setUp(self):
        ...
        patcher = mock.patch.object(cubie_precompile, "POLL_S", 0.02)
        patcher.start()
    ...
    def test_chunks_go_to_at_most_jobs_workers_at_once(self):
        ...
        self.assertEqual((driver.failed, driver.hung, driver.crashed, driver.broken), ([], [], [], 0))
    ...
    def test_a_hung_or_crashed_kernel_hands_the_rest_of_its_chunk_to_a_new_worker(self):
        status, driver, calls = self.drive(jobs=1, per_worker=4, hang_at=5)
        self.assertEqual(status, 0)
        self.assertEqual(driver.hung, [5])
        ...
        self.assertEqual(status, 0)
        self.assertEqual(driver.crashed, [7])
        self.assertEqual(driver.hung, [])
        ...
        self.assertEqual(driver.crashed, [9])
        self.assertEqual(driver.launched, 2)

    def test_failed_kernels_are_tallied_and_a_worker_dead_before_its_first_kernel_fails_the_pass(self):
        ...
        self.assertEqual(driver.broken, 1)
        # The other chunks still ran; the dead chunk's kernels are left to the runner.
        self.assertEqual(sorted(driver.compiled), [0, 1, 2, 6, 7, 8, 9])
        self.assertEqual(driver.launched, 4)

    def test_main_runs_the_driver_over_the_files_kernels(self):
        trials.write_jsonl(self.path, trials.build_trials([spec(8), spec(32), spec(8, algorithm="euler")]))
        with open(os.path.join(self.tmp, "plan.json"), "w") as handle:
