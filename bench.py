...
plan writes trials/<key>/<package>.jsonl and prints counts; run writes them under logs/<key>_<stamp>/ and drives each package's runner, a fresh cubie runner every 100 lines at a family boundary (<package>.part<N>.jsonl).
...
class Run:
    ...
    def run_package(self, package, trial_list, path):
        """Drive a package's runners over its trial file, a fresh runner per part of launch.RESTART_LINES lines of whole families; a build that crashed before a hard exit fails the package once every part ends."""
        parts = trials_mod.family_parts(trial_list, launch.RESTART_LINES.get(package))
        hard_exits, crashed, status = 0, [], 0
        for number, part in enumerate(parts, start=1):
            label = package if len(parts) == 1 else "{0} part {1}/{2}".format(package, number, len(parts))
            if len(parts) > 1:
                path = os.path.join(os.path.dirname(path), "{0}.part{1}.jsonl".format(package, number))
                trials_mod.write_jsonl(path, part)
            status, exits = self.run_part(package, label, part, path, crashed)
            hard_exits += exits
            if status is None:
                return
        self.finish(package, status, hard_exits, crashed)

    def run_part(self, package, label, trial_list, path, crashed):
        """(exit status, hard exits) of one runner over a trial file, re-invoking after every watchdog hard exit on what the abandonment leaves; status None once the package is recorded FAILED. Crashed builds join `crashed`."""
        logfile = package + ".log"
        stem = path[:-len(".jsonl")]
        hard_exits = 0
        while True:
            command = launch.runner_command(package, path, floor=self.args.floor)
            status = self.step("{0} ({1} trials)".format(label, len(trial_list)), logfile, command)
            if status == 0:
                return status, hard_exits
            if status != WATCHDOG_EXIT_CODE:
                self.record(package, "FAILED", "runner exit {0}".format(status), status)
                return None, hard_exits
            hard_exits += 1
            ...
            path = "{0}.retry{1}.jsonl".format(stem, hard_exits)
