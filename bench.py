...
--resume runs what the store lacks of each trial: a transfers row, a cold build time, a readable finals file, a valid optimize record, a row recorded after that record (an older one was timed under the settings the record replaced); --no-overwrite also reruns NaN rows and timed-out optimizes; --floor lets runners keep the lower finite time.
...
def continue_filter(trial_list, key, root, resume=False, no_overwrite=False, sources=source_hashes):
    """Trials still to run: each keeps the transfers completeness.audit finds lacking (every one behind a stale optimize record, those whose row predates the kernel's record, the last one alone for missing finals); a line without transfers never runs again."""
