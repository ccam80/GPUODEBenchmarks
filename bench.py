...
class Run:
    ...
    def run_package(self, package, trial_list, path):
        """Precompile cubie's kernels, then drive the package's runners over its trial file, a fresh runner per part of launch.RESTART_KERNELS kernels of whole families; a compile the precompile abandoned marks its lines in the trial file, which then run without an optimize; a build that crashed before a hard exit fails the package once every part ends."""
        if package == "cubie":
