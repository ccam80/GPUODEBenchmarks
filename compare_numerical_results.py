...
def parse_dataset_key(key):
    """Split a "<os>_<gpu>" directory name; gpu contains no underscores."""
    parts = key.split("_")
    if len(parts) != 2:
        return None
...
def build_comparison(group_label, datasets):
    ...
    md_path = os.path.join(group_dir(group_label), 'pairwise_comparisons.md')
    md_lines = []
...
def main():
    ...
    print("Groups written: " + ", ".join(g[0] for g in groups))
