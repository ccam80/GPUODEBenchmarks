...
- `runner_scripts/julia_systems.jl` is the one system module, for the kernels, the Float32 sweeps and the Float64 golden:
  - Builders take the element type: `_lorenz_entry(::Type{T})`, `_lorenz96_entry(::Type{T}, n)`, `_pleiades_entry(::Type{T})`, `_pollu_entry(::Type{T})`, `_ring_modulator_entry(::Type{T})`, `_ring_modulator_index2_entry(::Type{T})`, `_nand_gate_entry(::Type{T})`, `_build_entry(raw, ::Type{T}; u0map, golden_vars, consistent_u0 = false)`; helpers with literals take `T` first.
  - Every literal is `T(x)`, `ifelse` thresholds `zero(T)`, `@parameters` defaults `T(x)`; `_build_entry` returns `SMatrix{n,n,T}` mass matrix, `SVector{n,T}` u0 and `u0_for`; the Newton step in `_consistent_u0` stays Float64; `generate_jacobian` failing is an error; `mtkcompile(raw; split = false)` unchanged.
  - Registry `_ENTRIES::Dict{Tuple{String, DataType}, Any}`; `julia_system(problem, ::Type{T} = Float32)`; one-argument callers unchanged.
  - `cpu_problem(system, problem, p)`: `T = eltype(system.u0)`; `ODEFunction{true}(system.rhs!; jac = system.jac!)`, with `mass_matrix = Matrix{T}(system.mass_matrix)` when present; `ODEProblem{true}(f, Vector{T}(system.u0_for(T(p))), (zero(T), T(duration)), T[p])`.
  - Solve kwargs on every julia_cpu trial: `abstol`, `reltol`, the trial's `dt`, `dtmin`, `dtmax` when not NaN, `save_everystep = false`, `save_start = false`, `dense = false`, `maxiters = 10^8`, `verbose` off, no tstops. Finals are `sol.u[end][system.golden_index]`; `t_final = sol.t[end]`; `retcode = string(sol.retcode)` unless `Success`.
  - `GPU_ODE_JuliaKernels`: `_ENTRIES[(name, Float32)] = Base.structdiff(_ENTRY_BUILDERS[name](Float32), NamedTuple{(:sys,)})`; consumers unchanged.
- `runner_scripts/golden/verify_references.jl` on `julia_system(name, Float64)` through `cpu_problem`, each problem's `golden_algorithm` at `golden_tol`: pollu k1 = 0.35 against `POLLU_REF` to 1e-13; pleiades m1 = 1.0 against `PLEI_REF` to 1e-10; nand_gate c9 = 5e-5 against `NAND_REF` to 1e-8; ring_modulator Cs = 2e-12 against the Test Set II-3 table at t = 1e-3 to 1e-8; lorenz96 F = 8 Vern9 against RadauIIA5 to 1e-7; every problem Float32 rhs against Float64 rhs at 20 random states to 1e-5 relative; `@allocated rhs!` after one warm call is 0.
- `runner_scripts/tests/test_julia_systems.jl`, every problem, both element types: the entry builds; `eltype(u0) == T`; `mass_matrix` is `nothing` or `SMatrix{n,n,T}`; `rhs!` allocates 0 B; algebraic rows of `rhs!` at `u0_for(sweep_min)` below 1e-12 (Float64) and 1e-6 (Float32); `golden_index` is `states` distinct indices in `1:n`; `verify_references.jl` runs in the same invocation.
