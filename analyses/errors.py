...
class Errors:
    """Comparisons over one store; finals files, the golden rows and each traced row's three metrics are kept, trace files never are."""
    ...
    def interval_error(self, row):
        ...
        return self.trace_metrics(row)[1] if _has_traces(row) else NAN
    ...
    def error(self, row):
        ...
        if golden is None:
            return NAN
        if _has_traces(row) and _has_traces(golden):
            return self.trace_metrics(row)[0]
        if not (_has_finals(row) and _has_finals(golden)):
            return NAN
        return self.compare(row, golden)


def _pair(loaded_a, loaded_b):
    """(index_a, index_b) of the trajectories with the same grid value that neither loaded (values, states, errored) flags."""
    _, index_a, index_b = np.intersect1d(loaded_a[0], loaded_b[0], return_indices=True)
    keep = ~(loaded_a[2][index_a] | loaded_b[2][index_b])
...
def _peak_times(loaded):
    values, states, errored = loaded
    times = store_mod.trace_times()
    peaks = [_peaks(times, states[traj, :, 0].astype(np.float64)) if not errored[traj] else np.zeros(0)
             for traj in range(values.shape[0])]
    return values, peaks, errored


def _interval_error(loaded, loaded_golden):
    """Mean absolute inter-beat interval error in ms of the paired trajectories, per golden interval; NaN with no pair."""
    index_a, index_b = _pair(loaded, loaded_golden)
    _, peaks_a, _ = _peak_times(loaded)
    _, peaks_b, _ = _peak_times(loaded_golden)
    total, count = 0.0, 0
