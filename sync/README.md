...
A push copies this key and this machine's clocks files up and deletes nothing itself. It ships `sync/box_prune.py` to the box and holds the key's lock there (`box_prune.py serve` over ssh, the sftp remote's host, user and key) from its first copy until the upload has finished, then has the box prune: the `clocks/<key>_<stamp>.csv` logs a day or older that no result row under `key=<key>` on the box names are deleted, and the mirror and `logs/<run>/` drop the same ones. A failed upload releases the lock and prunes nothing; a results file the box cannot read stops the prune. By hand:
